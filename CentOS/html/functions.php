...
function usernameExists($conn, $username) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: https://10.0.21.4/signup_page.php?error=stmtfailed");
        exit();
    }
...
function createUser($conn, $user, $username, $pswd) {
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location: https://10.0.21.4/login_page.php?error=signedup");
    exit();
}
...
function loginUser($conn, $username, $pswd) {
    ...
    if ($usernameExists === false) {
        header("location: https://10.0.21.4/login_page.php?error=wronglogin");
        exit();
    }
    ...
    if ($pswddb == $pswd){
        ...
        header("location: https://10.0.21.4/start.php");
        exit();
    }
...
function getStationCode($conn, $user, $weatherstation) {
    ...
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: https://10.0.21.4/map.php?error=stmtfailed");
        exit();
    }
